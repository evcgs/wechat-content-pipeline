...
**❌ 错误做法（昨天就是这么做的，导致图片效果很差）：**
1. ❌ 跳过"结合文章内容二次生成详细配图描述"环节
2. ❌ 自己随意创建简化版提示词
3. ❌ 加入过多技术约束（颜色代码、像素高度、栅格系统等）
4. ❌ 使用"标题"这个词（AI会理解错）
5. ❌ 批量生成图片，一张一张确认

**✅ 正确做法（必须严格遵守！）：**
1. ✅ **必须使用文章/大纲中已有的详细配图描述**
2. ✅ **不要自己随意创建提示词**
3. ✅ **只保留画面内容描述和基本风格**
4. ✅ **把"标题"改成"文字"**
5. ✅ **一张一张生成并确认**
...
**✅ 正确做法：**
- 从步骤2到步骤9中间，任何一个步骤的文字内容有修改，都要往回去检查
- 同步修改图片的位置和详细描述
- 确保所有相关内容保持一致
...
**⚠️ 🔴 P0 检查必须全部确认后，才能开始生成图片！**

---

### 清单 2：文字修改后联动检查 ⭐⭐⭐（重要！）

**修改了任意步骤的文字内容后，必须检查以下内容：**
